...
mod app {
    ...
    use half_bridge::{
        ...
        hardware::{
            self, adc::ADC_CLOCK_MODE, DEADTIME_FALLING_TICKS, DEADTIME_RISING_TICKS,
            SYS_PLL_M_DIV, SYS_PLL_N_MUL, SYS_PLL_P_DIV, SYS_PLL_R_DIV, SYS_PLL_SOURCE,
        },
    };
    use stm32_hrtim::{
    ...
    fn foo(ctx: foo::Context) {
        ...
        let a = ctx.local.adc2.get_overrun_flag();
        let vout = ctx.local.adc2.current_sample();
        let b = ctx.local.adc2.get_overrun_flag();
        ctx.local.adc2.clear_overrun_flag();
        let c = ctx.local.adc2.get_overrun_flag();
